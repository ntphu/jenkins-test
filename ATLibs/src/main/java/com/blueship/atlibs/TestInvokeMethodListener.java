...
public class TestInvokeMethodListener implements IInvokedMethodListener{
	...
	public void afterInvocation(IInvokedMethod method, ITestResult result) {
		...
		if (method.isTestMethod()) {
			...
			List<Throwable> verificationFailures = ((TestCase)currentClass).action.getVerificationFailures();
			...
			if (verificationFailures.size() > 0) {
				...
				if (size == 1) {
					...
					for (int i = 0; i < size-1; i++) {
						...
						String fullStackTrace = org.testng.internal.Utils.stackTrace(t, false)[1];
